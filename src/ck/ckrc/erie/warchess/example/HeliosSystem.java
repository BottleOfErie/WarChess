...
public class HeliosSystem extends Chess {
    ...
    public static final String[] parts={"Elvis-Graham空间张量调谐器","Darius-Semiz驱动器","Wüthrich流变路由器"};
    public static final int build_cost =50,max_hp=10;
    public static final int initTime=3,attTime=5,damage=1000,attCost=10,stopCost=5;
    ...
    public Node showPanel() {
        //TODO draw cool panel background
        ...
        if(countdown==-1) {
            status.setText("巡航中");
            button=new Button("引导攻击术式");
            button.setOnAction(actionEvent-> {
                var currentEnergy=(Integer)Main.currentGameEngine.getPlayer(teamFlag).getStatus(Miner.energyKey);
                if(currentEnergy<attCost)
                    status.setText("没有足够的能量");
                else {
                    Main.currentGameEngine.getPlayer(teamFlag).setStatus(Miner.energyKey, currentEnergy - attCost);
                    countdown = attTime;
            ...
            status.setText("正在引导术式，剩余" + countdown + "时间单位");
            button=new Button("干扰术式成型");
            button.setOnAction(actionEvent -> {
                var currentEnergy=(Integer)Main.currentGameEngine.getPlayer(teamFlag).getStatus(Miner.energyKey);
                if(currentEnergy<stopCost)
                    status.setText("没有足够的能量");
                else {
                    Main.currentGameEngine.getPlayer(teamFlag).setStatus(Miner.energyKey, currentEnergy - stopCost);
                    countdown = -1;
                    status.setText("巡航中");
                }
            });
    ...
    public static Node showData(){
        //TODO draw cool data background
    ...
    public void roundBegin() {
        ...
        Random r=new Random();
        next_target_x =r.nextInt(Map.MapSize);
        next_target_y =r.nextInt(Map.MapSize);
    }
